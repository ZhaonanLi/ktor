...
internal class TLSServerHello(
    ...
    lateinit var hashAndSignAlgorithms: List<HashAndSign>

    init {
        parseExtensions()
    }

    private fun parseExtensions() = extensions.forEach {
        when (it.type) {
            TLSExtensionType.SIGNATURE_ALGORITHMS -> {
                hashAndSignAlgorithms = it.packet.parseSignatureAlgorithms()
            }
        }
    }
}
